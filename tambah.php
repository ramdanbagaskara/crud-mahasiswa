...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO mahasiswa (nim, nama, jurusan, semester, jenis_kelamin) VALUES (?, ?, ?, ?, ?)");
        if (!$stmt) {
            $pesan = '<div class="notif error">❌ Prepare statement gagal: ' . mysqli_error($conn) . '</div>';
        } else {
            mysqli_stmt_bind_param($stmt, "ssssi", $nim, $nama, $jurusan, $jk, $semester);
            // Koreksi urutan bind: semester int di posisi ke-4
            mysqli_stmt_close($stmt);

            // Bind ulang dengan urutan benar
            $stmt = mysqli_prepare($conn, "INSERT INTO mahasiswa (nim, nama, jurusan, semester, jenis_kelamin) VALUES (?, ?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, "sssis", $nim, $nama, $jurusan, $semester, $jk);

            if (mysqli_stmt_execute($stmt)) {
                header('Location: index.php?tambah=1');
                exit;
            } else {
                $pesan = '<div class="notif error">❌ Gagal menyimpan data: ' . mysqli_stmt_error($stmt) . '</div>';
            }
            mysqli_stmt_close($stmt);
        }
    }
...
<div class="card">
    ...
    <form method="POST" action="tambah.php">
        <div class="form-group">
            ...
            <input type="text" id="nim" name="nim" placeholder="Contoh: 2021001234"
                   value="<?= htmlspecialchars($_POST['nim'] ?? '') ?>" maxlength="20">
        </div>
        <div class="form-group">
            <label for="nama">Nama Lengkap</label>
            <input type="text" id="nama" name="nama" placeholder="Nama lengkap mahasiswa"
                   value="<?= htmlspecialchars($_POST['nama'] ?? '') ?>" maxlength="100">
        </div>
        <div class="form-group">
            ...
            <input type="text" id="jurusan" name="jurusan" placeholder="Contoh: Teknik Informatika"
                   value="<?= htmlspecialchars($_POST['jurusan'] ?? '') ?>" maxlength="100">
        </div>
        <div class="form-row">
            <div class="form-group">
                <label for="semester">Semester</label>
                <input type="number" id="semester" name="semester" placeholder="1–14" min="1" max="14"
